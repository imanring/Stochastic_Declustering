...
class stochastic_declustering:
    ...
    def bg_est(self):
        ...
        self.skde.set_params(bandwidth= self.prob_background.sum()**(-1/6)*s)#prob_background.sum()
    ...
    def Lamb_t(self,t):
        ...
        for i in range(len(t)):
            ...
            trig[i] = r_0*(M*expon.cdf(t[i]-self.events[,2],scale=b)).sum()
        return (bg+trig)
    ...
    def delta(self,x):
        ...
        s_int = self.skde.score_samples(s)
        if self.iht:
